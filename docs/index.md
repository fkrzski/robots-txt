...
- **[Guide](/robots-txt/guide)** — how global and crawler-specific rules compose, how the
  output is ordered, and how to write the file to disk.
- **[API reference](/robots-txt/api-reference)** — every method with its signature,
  validation rules, and verified output.
- **[Crawlers](/robots-txt/crawlers)** — the full `CrawlerEnum` list mapped to official
  user-agent strings.
